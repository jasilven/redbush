...
fun! s:exit(...)
  if s:jobid != 0
    ...
    "   let lines = getbufline(g:redbush_logfile, 0 ,"$")
    "   let tmstamp = strftime("%H:%M:%S %b %d %Y")
    "   call setbufline(g:redbush_logfile, len(lines)+1, ";; End   " . tmstamp)
    " endif
  endif
  " call s:logbuf_hide()
endf
...
if !exists('g:redbush_nrepl_session_id')
  let g:redbush_nrepl_session_id = 'UNINITIALIZED'
endif
